...
if len(bitly1)>6 or len(bitly1)<4:
    ...
    for p in urllist:
        ...
        try:
            ...
            elif 410 <= e.code <= 451 or e.code > 511:
...
print "   "
print "You can either: proceed witht the script to expand each of the bit.ly links to see the full URL or close the script and export the list of bit.ly URLs to a txt file"
while True:
    expand = raw_input("Do you wish to unshorten the scripts? Y/N: ")
    if expand.lower() not in ('Y', 'y','N', 'n'):
        print("Error. Please type Y if you wish to unshorten. Type N if you wish to close the script")
    else:
        break
if expand == 'N' or expand == 'n':
    print "Terminating script. TXT export will be available in future versions"
else:
    print "Preparing to unshorten the links. Wait"
    sleep(3)
    for p in urllist:
        print urllib2.urlopen(p).geturl()
    print "unshorten finished"
